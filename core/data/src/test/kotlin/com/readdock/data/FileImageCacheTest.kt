...
class FileImageCacheTest {
    ...
    fun `stores reads and clears image bytes`() {
        val directory = Files.createTempDirectory("readdock-image-cache").toFile()
        try {
    ...
    fun `evicts oldest files over the configured limit`() {
        val directory = Files.createTempDirectory("readdock-image-eviction").toFile()
        try {
    ...
    fun `rejects one entry larger than the per-file limit`() {
        val directory = Files.createTempDirectory("readdock-image-entry-limit").toFile()
        try {
